...
- Morph factor `k = saturate((dist − start_d) / (end_d − start_d))` per vertex, where `end_d` is the split distance of depth d and `start_d = end_d × (1 − MorphStartFraction)`. Ranges nest (`end_{d−1} = 2·end_d`), so at any boundary between depths d and d−1 the fine side is at `k = 1` and the coarse side at `k = 0`: no crack, no T-junction, no stitching.
  - **Corrected during M3b: `end_d` is the split distance of depth d−1, not of depth d.** A chunk must be fully morphed where its *parent* takes over, which is the parent's split distance. Its own split distance is where it hands over to its *children*; finishing the morph there leaves a chunk drawing its parent's geometry while its coarser neighbour, also finished, draws its grandparent's. Measured against a simulated selection, the original definition put **100% of cross-depth boundary samples exactly 1.0 effective LOD apart** — a seam along every boundary in the world and a full level of pop at every swap, which is what M3b's first cut shipped. `LodMathTests.MorphCompletesWhereTheParentTakesOver` and `LodBoundaryTests` pin it; the latter also reproduces the broken version.
  ...
  - **And node extent tightens that bound much further, which decides `SplitFactor`.** Selection uses a per-node bounding-sphere distance while the morph uses a per-vertex distance, and a coarse leaf's own patch reaches past its sphere distance. The shared point of a boundary therefore sits at up to `Hysteresis + SizeSpread / SplitFactor` times the coarse node's split distance, where `SizeSpread` is `SurfaceMath.MaxNodeSizeSpread` (π/2 for a tangent-adjusted cube sphere, 1 for a plane grid). Requiring the coarse side still to be at `k = 0` there gives

    ```
    MorphStartFraction ≤ 1 − (Hysteresis + SizeSpread / SplitFactor) / 2
    ```

    Measured, this is correct and slightly conservative. Its consequence is that **`SplitFactor = 2` is not usable with a per-vertex morph**: on a cube sphere with any hysteresis at all it allows a morph window under 1% wide, which is a pop by another name. The defaults are therefore `SplitFactor = 3`, `HysteresisFactor = 1.1`, `MorphStartFraction = 0.15`, and `LodMath.Create` throws on a combination that exceeds the ceiling. Chunk cost scales with `SplitFactor²`: the shipped configuration wants **1358 chunks, 1020 of them leaves**, against 766/576 at `SplitFactor = 2`.
