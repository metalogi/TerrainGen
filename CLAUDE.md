...
- `Core/Surface/`: `NodeId`, `Edge`, `RootQuad`, `SurfaceDef`, `EdgeLink`, `SurfaceMath` (M1 topology layer: cube-sphere, plane grid, cylinder)
...
- **Two `SurfaceType` enums exist and the name is ambiguous inside `Sonoma.Core`.** `Sonoma.Core.Surface.SurfaceType` is `{ PlaneGrid, CubeSphere, Cylinder }`; the older `Sonoma.Core.CoordinateSpace.SurfaceType` is `{ Plane, Sphere, Cylinder }`. Any file importing both namespaces gets CS0104 on the bare name `SurfaceType` — which is exactly the set of files M2's switchover touches. Qualify it, and do **not** "fix" the ambiguity with a cast: the two happen to line up numerically (0, 1, 2), so a cast compiles, runs, and silently equates the old unbounded `Plane` with the new tiled `PlaneGrid` and the UV-sphere with the cube-sphere. The old enum dies with `CoordinateTransform` in M2.
