...
- **Seams are exact within a root quad and merely bounded across one.** Across a cube edge the two faces reach the shared edge by different parameterisations, so the surface points differ by 2.067e-16 relative (1.32 nm at Earth radius). `SonomaRevisedPlan.md` §5 asks for bit-identical heights "including across a cube edge"; that half is unachievable. Skirts cover it.
